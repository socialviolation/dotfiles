...
def reason(path, findings):
    first = findings[0]
    return "BLOCKED \u2014 %d %s in %s; the tolerance here is %d.\n\n%s\n\n%s" % (
        len(first["lines"]), first["kind"], path, first["limit"], listing(findings), BODY)


def reason_after(path, findings):
    first = findings[0]
    return (
        "REJECTED \u2014 %d %s are now sitting in %s; the tolerance there is %d. A shell "
        "command wrote them, so the edit hook never got to stop it. Writing files through "
        "the shell does not exempt you from this rule.\n\n%s\n\nDelete those lines now, "
